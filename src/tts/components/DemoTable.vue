...
const loadDemos = async () => {
  ...
  try {
    ...
    const entries = await Promise.all(
      (data.demos || []).map(async ({ speaker }) => {
        try {
          const txtRes = await fetch(`/api/demo/file/${encodeURIComponent(speaker)}.txt`);
          const text = txtRes.ok ? (await txtRes.text()).trim() : '';
          return {
            speaker,
            text,
            audioUrl: `/api/demo/file/${encodeURIComponent(speaker)}.wav`,
          };
        } catch {
          return null;
        }
    ...
    demos.value = entries.filter(Boolean);
  } catch (error) {
